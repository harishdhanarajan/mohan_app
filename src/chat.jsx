// MYT — AI Chat assistant (admin only).
...
export function ChatAssistant({ state, onOpenTask, onClose, initialMessages, onMessagesChange, initialInput = "", onInputChange }) {
  const [messages, setMessagesInternal] = React.useState(initialMessages || [
    {
      role: "assistant",
  ...
  const [input, setInputInternal] = React.useState(initialInput);
  const [loading, setLoading] = React.useState(false);
  const bodyRef = React.useRef(null);
  const setMessages = React.useCallback((nextMessages) => {
    setMessagesInternal(nextMessages);
    onMessagesChange?.(nextMessages);
  }, [onMessagesChange]);
  const setInput = React.useCallback((nextInput) => {
    setInputInternal(nextInput);
...
User question: ${text}`;
    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ prompt: systemPreface })
      });
      ...
      const text = e.message?.includes("OPENAI_API_KEY")
        ? "OpenAI is not configured yet. Add OPENAI_API_KEY to .env or .env.local in the project root, then restart the dev server."
