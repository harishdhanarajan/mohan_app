...
function TaskListView({ tasks, state, onOpen, currentUser, onToggleDone }) {
  if (!tasks.length) {
  ...
  return (
    <div className="tasklist">
      ...
      {tasks.map(t => {
        ...
        const canCheck = currentUser.role === "admin" || currentUser.id === t.assigneeId;
        return (
...
function KanbanView({ tasks, state, onOpen }) {
  const cols = [
    { id: "todo", label: "To Do" },
          ...
          <div className="kanban-col" key={col.id}>
            ...
            <div className="kanban-col-body">
              {colTasks.map(t => {
                ...
                return (
                  <div key={t.id} className={`kanban-card prio-${prio}-edge`} onClick={() => onOpen(t.id)}>
...
function CalendarView({ tasks, state, onOpen }) {
...
Object.assign(window, { TaskListView, KanbanView, CalendarView });
