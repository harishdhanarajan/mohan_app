...
function StatusDonut({ state }) {
  ...
  const r = 56;
  const C = 2 * Math.PI * r;
  const segs = window.MYT.STATUS_ORDER.map(s => {
    const v = counts[s] || 0;
    const len = (v / total) * C;
          ...
          <div className="donut-legend">
            {window.MYT.STATUS_ORDER.map(s => (
              <div key={s} className="legend-row">
                ...
                <div>{window.MYT.STATUS_LABELS[s]}</div>
        ...
        {top.map((a, i) => {
          ...
          return (
            <div className="feed-row" key={i}>
              ...
              <div className="feed-time">{window.MYT.formatRelTime(a.ts)}</div>
            </div>
          );
        })}
      </div>
...
function NoCommentsWidget({ state, onOpen }) {
  const tomorrow = window.MYT.dayOffset(1);
...
function Dashboard({ state, onOpen, layout }) {
  const today = window.MYT.todayISO();
