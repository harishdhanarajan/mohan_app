...
function Avatar({ user, size }) {
  ...
  return React.createElement("div", {
    className: klass,
    style: { background: window.MYT.colorFor(user.id) },
    title: user.name
  }, window.MYT.initials(user.name));
}
...
function PriorityBadge({ priority }) {
  ...
  return (
    <span className={`badge ${k}`}>
      ...
      {window.MYT.PRIORITY_LABELS[priority]}
    </span>
  );
}
...
function StatusBadge({ status }) {
  ...
  return (
    <span className={`badge ${map[status] || "badge-status"}`}>
      ...
      {window.MYT.STATUS_LABELS[status]}
    </span>
  );
}

function DueCell({ task }) {
  const cls = window.MYT.dueClass(task.dueDate, task.status);
  return (
    <span className={`due-cell ${cls}`}>
      <Icon name="clock" size={12} />
      {window.MYT.formatDue(task.dueDate)}
    </span>
  );
}

Object.assign(window, { Avatar, Icon, Badge, PriorityBadge, StatusBadge, DueCell });
